...
def get_tickers(partha_account,yesterday_df):
    ...
    #print(tabulate(day_df,headers='keys',tablefmt='psql'))
    up_df=day_df[(day_df['D_C_MIN'] < 50) & (day_df['D_C_MIN'] > 0.5) & (day_df['D_MIN']>-0.5)] 
    ...
    down_df=day_df[(day_df['D_C_MAX'] > -50) & (day_df['D_C_MAX'] < -0.5) & (day_df['D_MAX']<0.5)]
...
def get_data(new_data=False):
    ...
    for coin in busd_list:
        klines = partha_account.futures_historical_klines(
            coin, partha_account.KLINE_INTERVAL_30MINUTE, "30 minute ago UTC")
        if len(klines) > 0:
    ...
    coin_data_df.to_csv("previous_30min_data.csv")
