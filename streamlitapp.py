...
def get_tickers(partha_account, df_ins_tick):
    ...
    table_data = table_data.rename(columns={
                                   'openPrice': 'D_OPEN', 'highPrice': 'D_HIGH', 'lowPrice': 'D_LOW', 'price': 'C_PRICE'})
    ...
    table_data['D_HIGH'] = np.maximum(
        table_data['D_HIGH'], table_data['C_PRICE'])
    table_data['D_LOW'] = np.minimum(
        table_data['D_LOW'], table_data['C_PRICE'])
    table_data['D_MAX'] = (table_data['D_HIGH'] -
                           table_data['D_OPEN'])/table_data['D_OPEN'] * 100
    table_data['D_MIN'] = (table_data['D_LOW'] -
                           table_data['D_OPEN'])/table_data['D_OPEN'] * 100
    table_data['PCT'] = ((table_data['C_PRICE'] - table_data['D_LOW']) /
                         (table_data['D_HIGH'] - table_data['D_LOW'])) * 100
    ...
    table_data['SPREAD'] = (table_data['D_HIGH'] -
                            table_data['D_LOW'])/table_data['D_LOW'] * 100

    table_data['D_CHANGE'] = (
        table_data['C_PRICE'] - table_data['D_OPEN'])/table_data['D_OPEN'] * 100
    table_data['D_C_MIN'] = (table_data['C_PRICE'] -
                             table_data['D_LOW'])/table_data['D_LOW'] * 100
    table_data['D_C_MAX'] = (table_data['C_PRICE'] -
    ...
    day_df = table_data[['symbol', 'D_OPEN', 'D_HIGH', 'D_LOW', 'C_PRICE',
                         'D_MAX', 'D_MIN', 'D_C_MAX', 'D_C_MIN', 'PCT', 'SPREAD', 'D_CHANGE']]
...
while True:
    ...
    table_data = pd.merge(df_tickers, ins_tick, on='symbol', how='left')
    table_data = table_data.rename(columns={
                                   'openPrice': 'D_OPEN', 'highPrice': 'D_HIGH', 'lowPrice': 'D_LOW', 'price': 'C_PRICE'})
    # print(tabulate(table_data, headers='keys', tablefmt='psql'))
