...
CSV_FILE = 'total_data.csv'  # Hardcoded file name for the first tab
CSV_FILE_2 = 'up_df.csv'  # Hardcoded file name for the second tab
CSV_FILE_3 = 'down_df.csv'  # Hardcoded file name for the third tab
...
def create_tab_layout(csv_file):
    data, header_list = read_csv_file(csv_file)

    # ------ Tab Layout ------
    layout = [
        [sg.Text('Click a heading to sort on that column or enter a filter and click a heading to search for matches in that column')],
        [sg.Text(f'{len(data)} Records in table', font='_ 18')],
        [sg.Text(k='-RECORDS SHOWN-', font='_ 18')],
        [sg.Text(k='-SELECTED-')],
        [sg.T('Filter:'), sg.Input(k='-FILTER-', focus=True, tooltip='Not case sensitive\nEnter value and click on a column header'),
...
def main():
    ...
    tab1_layout = create_tab_layout(CSV_FILE)
    tab2_layout = create_tab_layout(CSV_FILE_2)
    tab3_layout = create_tab_layout(CSV_FILE_3)
    ...
    layout = [
        [sg.TabGroup([
            [sg.Tab('Tab 1', tab1_layout, key='-TAB1-', tooltip='Table 1')],
            [sg.Tab('Tab 2', tab2_layout, key='-TAB2-', tooltip='Table 2')],
            [sg.Tab('Tab 3', tab3_layout, key='-TAB3-', tooltip='Table 3')],
        ])],
    ]
    ...
    original_data = None  # save a copy of the data
    # ------ Event Loop ------
    while True:
        ...
        if event in (sg.WIN_CLOSED, 'Exit'):
            break
        if values['-TABLE-']:  # Show how many rows are selected
